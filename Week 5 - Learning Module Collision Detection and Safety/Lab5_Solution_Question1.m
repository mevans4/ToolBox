...
function [  ] = Lab5_Solution_Question1(  )
clf
...
while ~isempty(find(0 < ufoFleet.healthRemaining,1))
    ...
    goalJointState = GetGoalJointState(blasterRobot,ufoFleet,1);
...
function goalJointState = GetGoalJointState(blasterRobot,ufoFleet,solutionToUse)
    ...
    elseif solutionToUse == 2
        goalJointState = blasterRobot.model.getpos();
        ...
        % yet)
        actualTr = blasterRobot.model.fkine(goalJointState).T;
        ufoBase = ufoFleet.model{targetUFOIndex}.base.T;
        aVector =  ufoBase(1:3, 4)' - actualTr(1:3,4)';
        % Make a unit vector
        ...
        while maxAllowableDistError < minDistError && 0 < iKineAlpha
            ...
            try
                %candidateJointState = blasterRobot.ikine(tr, 'q0', bestJointState, 'mask', [0,0,0,1,1,0], 'quiet', 'alpha', iKineAlpha);
                %candidateJointState = blasterRobot.model.ikine(tr, 'q0', bestJointState, 'mask', [0,0,0,1,1,0], 'alpha', iKineAlpha, 'forceSoln');
                candidateJointState = blasterRobot.model.ikine(tr, 'q0', bestJointState, 'mask', [0,0,0,1,1,0], 'alpha', iKineAlpha);
            catch
                qEmpty = true;
                break
            end
            
            % NOTE: If robot can not physically reach the pose given to ikine,
            % or ikine fails to converge to a solution, break out of the
            % while loop as none of the below computations can be
            % completed.
            if isempty(candidateJointState)
                qEmpty = true;
                break
            end
            
            % Fix joint limits if needed
            if ~all(blasterRobot.model.qlim(:,1) < candidateJointState'  &  candidateJointState' < blasterRobot.model.qlim(:,2))
                lessThanMinIndex = candidateJointState' < blasterRobot.model.qlim(:,1);
                greaterThanMaxIndex = blasterRobot.model.qlim(:,2) < candidateJointState';
                candidateJointState(lessThanMinIndex) = blasterRobot.model.qlim(lessThanMinIndex,1) + 10*pi/180;
                candidateJointState(greaterThanMaxIndex) = blasterRobot.model.qlim(greaterThanMaxIndex,2) - 10*pi/180;               
                display('Not in joint limits, so fixing');
            end

            actualTr = blasterRobot.model.fkine(candidateJointState).T;
        ...
        elseif all(blasterRobot.model.getpos() == 0)
            %disp('all zero already')
            goalJointState = (rand(1,6)-0.5) * 30*pi/180;
        % If ikine doesn't return a soln, choose a random pose
        elseif qEmpty
            disp('ikine empty')
            goalJointState = (rand(1,6)-0.5) * 30*pi/180;
